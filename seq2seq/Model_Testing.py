...
def translate_random_test_example(
    ...
    print(
        f"Levenshtien Distance: {levenshtein_distance(expected_output_tokens, output_tokens)}"
    )
...
def run(agn_input):
    ...
    agnostic_vocab = Vocabulary(
        [d["agnostic_tokens"] for d in train_data + val_data + test_data]
    )
    semantic_vocab = Vocabulary(
        [d["semantic_tokens"] for d in train_data + val_data + test_data]
    )
