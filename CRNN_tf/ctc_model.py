...
def ctc_crnn(params):
    ...
    feature_dim = params["conv_filter_n"][-1] * (
        params["img_height"] // height_reduction
    )
    features = tf.transpose(x, perm=[0, 2, 1, 3])  # [batch, width, height, channels]
    features = tf.reshape(
        features, [tf.shape(features)[0], -1, feature_dim]
    )  # [batch, width, features]
    ...
    targets = tf.sparse.SparseTensor(
        indices=tf.keras.layers.Input(
            name="target_indices", shape=(None, 2), dtype=tf.int64
        ),
        values=tf.keras.layers.Input(
            name="target_values", shape=(None,), dtype=tf.int32
        ),
        dense_shape=tf.keras.layers.Input(
            name="target_dense_shape", shape=(2,), dtype=tf.int64
        ),
    )

    ctc_loss = tf.keras.backend.ctc_batch_cost(
        y_true=targets, y_pred=logits, input_length=seq_len, label_length=tf.constant(0)
    )
